...
### Backend contract
Single endpoint: `POST /api/scan` with `{ barcode }` in the body, returns `{ success, barcode, message }`. `server/index.js` is intentionally a thin echo — no Open Food Facts lookup, no grading logic, no persistence. `src/api/scan.js` is the single fetch wrapper the frontend uses to call it (`API_BASE_URL` is hardcoded to `http://localhost:4000`).
